...
function parsingDataFromInput(req, res, next) {
  ...
  axios(url)
    ...
    .then(data => {
      ...
      let a = Promise.allSettled(array).then(data => data.filter(x => x.status === 'fulfilled')).catch(err => console.log(err))
      a.then(x => {
        const dataFinal = [];
        x.forEach(element => {
          dataFinal.push({ companyName: element.value.data.name, companyLogo: element.value.data.logo_urls, companyId: element.value.data.id })
          return { companyName: element.value.data.name, companyLogo: element.value.data.logo_urls, companyId: element.value.data.id }
        })
        res.json(dataFinal)
      })
