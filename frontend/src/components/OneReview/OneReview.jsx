...
export default function OneReview() {
  ...
  useEffect(() => {
    // if (reviews.length !== 0) {
    //   console.log(reviews);
    //   thisReview = reviews.find((elem) => elem._id == id))
    //   console.log(thisReview)
    //   setOnePost(thisReview)
    // }
    // else {
dispatch(getAllFetch())
    // thisReview = reviews.find(elem => {
    //   console.log(elem);
    //   console.log(id);
    //   return (elem._id == id)
    // })
    console.log(allPost);
    setOnePost(allPost.find((elem) => elem._id == id.id))
}, [id])
...
return (
  <div className='container container-main'>
    ...
    <div>Файлы с собеседования: <img src={onePost.image} /></div>
    <div> {onePost.setteled ? "Усторился" : "Не устроился"}</div>
    <div> {(onePost.author._id == user.id) ?
      <button>Редактировать</button> : <button>Like</button>
    }
